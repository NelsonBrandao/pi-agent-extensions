...
async function acquireLock(todosDir: string, id: string, ctx: ExtensionContext): Promise<(() => Promise<void>) | { error: string }> {
...
async function withTodoLock<T>(todosDir: string, id: string, ctx: ExtensionContext, fn: () => Promise<T>): Promise<T | { error: string }> {
...
async function deleteTodo(todosDir: string, id: string, ctx: ExtensionContext): Promise<TodoRecord | { error: string }> {
  const validated = validateTodoId(id);
  if ("error" in validated) {
    return { error: validated.error };
  }
    ...
    handler: async (args, ctx) => {
      ...
      await ctx.ui.custom<void>((tui, theme, _kb, done) => {
        ...
        selector = new TodoSelectorComponent(
          ...
          (todo, action) => {
            const title = todo.title || "(untitled)";
            nextPrompt = action === "refine" ? buildRefinePrompt(todo.id, title) : `work on todo ${formatTodoId(todo.id)} "${title}"`;
            done();
          },
        );
