...
// ── Finding extraction & todo creation ──────────────────────────────────────

interface ReviewFinding {
  priority: string; // "P0" | "P1" | "P2" | "P3"
  title: string;
  detail: string; // multi-line detail (includes the title line + subsequent lines)
}

/**
 * Extract individual findings from review output text.
 * Finds the "Findings" section and parses each tagged finding line along with
 * any continuation lines that belong to it.
 */
function extractFindings(messageText: string): ReviewFinding[] {
  const lines = messageText.split(/\r?\n/);
  const bounds = getFindingsSectionBounds(lines);
  const candidateLines = bounds ? lines.slice(bounds.start, bounds.end) : lines;
  ...
  const flushCurrent = () => {
    if (currentFinding) {
      currentFinding.detail = detailLines.join("\n").trim();
  ...
  for (const line of candidateLines) {
    if (/^\s*```/.test(line)) {
      inCodeFence = !inCodeFence;
      if (currentFinding) detailLines.push(line);
      continue;
    }
    if (inCodeFence) {
      if (currentFinding) detailLines.push(line);
    ...
    if (isLikelyFindingLine(line)) {
      ...
      const priorityMatch = line.match(/\[(P[0-3])\]/i);
      if (!priorityMatch) continue;

      const priority = priorityMatch[1].toUpperCase();
      // Strip markdown list/heading markers and the priority tag to get a clean title
      let title = line
        .replace(/^\s*(?:[-*+]|(?:\d+)[.)]|#{1,6})\s+/, "")
        .replace(/\*\*\[P[0-3]\]\*\*/i, "")
        .replace(/\[P[0-3]\]/i, "")
      ...
      currentFinding = { priority, title, detail: "" };
      detailLines.push(line);
    } else if (currentFinding) {
      // Continuation line (indented or non-empty body belonging to the current finding)
      detailLines.push(line);
    }
  }
...
// ── Todo file helpers (compatible with todos.ts format) ─────────────────────
...
function getReviewTodosDir(cwd: string): string {
...
function getReviewTodoPath(todosDir: string, id: string): string {
  return path.join(todosDir, `${id}.md`);
}

async function generateReviewTodoId(todosDir: string): Promise<string> {
  for (let attempt = 0; attempt < 10; attempt++) {
    const id = crypto.randomBytes(4).toString("hex");
    const todoPath = getReviewTodoPath(todosDir, id);
    if (!existsSync(todoPath)) return id;
  }
  throw new Error("Failed to generate unique todo id");
}

function serializeReviewTodo(todo: TodoFileRecord): string {
...
async function createTodosFromFindings(
  ...
  for (const finding of findings) {
    const id = await generateReviewTodoId(todosDir);
    const filePath = getReviewTodoPath(todosDir, id);
    const todo: TodoFileRecord = {
      id,
      title: `[${finding.priority}] ${finding.title}`,
      tags: ["review", finding.priority.toLowerCase()],
      status: "open",
      created_at: new Date().toISOString(),
      body: finding.detail,
    };
    await fs.writeFile(filePath, serializeReviewTodo(todo), "utf8");
    created++;
  }

  return { created, todosDir };
}
 ...
 */
async function buildReviewPrompt(pi: ExtensionAPI, target: ReviewTarget, options?: { includeLocalChanges?: boolean }): Promise<string> {
...
export default function reviewExtension(pi: ExtensionAPI) {
  ...
  async function runLoopFixingReview(ctx: ExtensionCommandContext, target: ReviewTarget): Promise<void> {
    ...
    try {
      ctx.ui.notify("Loop fixing enabled: using Empty branch mode and cycling until no blocking findings remain.", "info");
      ...
      ctx.ui.notify(`Loop fixing stopped after ${REVIEW_LOOP_MAX_ITERATIONS} passes (safety limit reached).`, "warning");
    } finally {
...
5. End with: fixed items, deferred/skipped items (with reasons), and verification results.`;
  ...
  /**
   * Extract findings from the latest review and create todos for each one.
   * Can optionally proceed with an end-review action afterwards.
   */
  async function createTodosFromReview(ctx: ExtensionCommandContext): Promise<void> {
    const snapshot = getLastAssistantSnapshot(ctx);
    if (!snapshot || !snapshot.text.trim()) {
      ctx.ui.notify("No review output found. Run a review first.", "warning");
      return;
    }

    const findings = extractFindings(snapshot.text);
    if (findings.length === 0) {
      ctx.ui.notify("No findings detected in the latest review output.", "info");
      return;
    }
    ...
    if (!confirmed) {
      ctx.ui.notify("Cancelled. No todos created.", "info");
      return;
    }

    try {
      const { created, todosDir } = await createTodosFromFindings(findings, ctx.cwd);
      ctx.ui.notify(`Created ${created} todo(s) in ${path.relative(ctx.cwd, todosDir) || todosDir}`, "info");
    } catch (error) {
      ctx.ui.notify(`Failed to create todos: ${error instanceof Error ? error.message : String(error)}`, "error");
      return;
    }

    // After creating todos, ask if the user also wants to end the review
    if (reviewOriginId) {
      const followUp = await ctx.ui.select("Also end the review?", [
        "Return only",
  ...
  async function runEndReview(ctx: ExtensionCommandContext): Promise<void> {
    ...
    try {
      ...
      if (choice === "Create todos for findings") {
        await createTodosFromReview(ctx);
        return;
      }

      const action: EndReviewAction =
        choice === "Return and fix findings" ? "returnAndFix" : choice === "Return and summarize" ? "returnAndSummarize" : "returnOnly";
